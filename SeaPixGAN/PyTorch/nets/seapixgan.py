...
class SeaPixGAN_Nets:
    def _init_(self, base_model='pix2pix'):
...
class GeneratorSeaPixGan(nn.Module):
    def _init_(self):
        super(GeneratorSeaPixGan, self)._init_()
...
class _EncodeLayer(nn.Module):
    def _init_(self, in_size, out_size, batch_normalize=True):
        super(EncodeLayer, self).__init_()
...
class _DecodeLayer(nn.Module):
    def _init_(self, in_size, out_size, dropout=False):
        super(DecodeLayer, self).__init_()
        layers = [
...
class DiscriminatorSeaPixGan(nn.Module):
    def _init_(self, in_channels=3):
        super(DiscriminatorSeaPixGan, self)._init_()
